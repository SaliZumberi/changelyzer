...
export class CardItemComponent implements OnInit {
  ...
  fetchDiffExamples() {
    ...
    if (!this.isLoading) {
      ...
      this.requestDataFromMultipleSources().subscribe(([diffs, diffsWhat, diffsWhere]) => {
        this.diffs = diffs;
  ...
  getDiffExamples() {
    if (this.selectedChangeType == 'all') {
      this.diffExamples = _.shuffle(this.diffs.filter(diff => {
        return _.includes(diff.vectorized, this.data.index);
      }));
    } else if (this.selectedChangeType == 'what') {
      this.diffExamples = _.shuffle(this.diffsWhat.filter(diff => {
        return _.includes(diff.vectorized, this.data.index);
      }));
    } else if (this.selectedChangeType == 'where') {
      this.diffExamples = _.shuffle(this.diffsWhere.filter(diff => {
